 ...
 * org.apache.poi.ss.usermodel.Picture; import
 * org.apache.poi.ss.usermodel.PictureData; import
 ...
 * org.apache.poi.xssf.usermodel.XSSFPictureData; import
 * org.apache.poi.xssf.usermodel.XSSFShape; import
 * org.apache.poi.xssf.usermodel.XSSFWorkbook; import
 * org.springframework.stereotype.Component; import
 * com.webtoeic.entities.CauHoiBaiTapNghe;
 * 
 * @Component public class ExcelUtil {
 * 
 * public List<CauHoiBaiTapNghe> getListCauHoiFromFileExcel(InputStream
 * excelFile) {
 * 
 * try { List<CauHoiBaiTapNghe> listCauHoi = new ArrayList<>(); Workbook
 ...
 * iterator.next(); Cell firstCell = firstRow.getCell(0);
 * System.out.println(firstCell.getStringCellValue()); XSSFDrawing dp =
 * (XSSFDrawing) datatypeSheet.createDrawingPatriarch(); List<XSSFShape> pics =
 * dp.getShapes();
 * 
 * while (iterator.hasNext()) { Row currentRow = iterator.next();
 ...
 * 
 * XSSFClientAnchor clientAnchor = inpPic.getClientAnchor(); PictureData pict =
 * inpPic.getPictureData(); byte[] data = pict.getData(); if(
 * clientAnchor.getCol1() == 8 && clientAnchor.getRow1() ==
 * currentRow.getRowNum()) { byte[] pictureData = data;
 * cauHoi.setPhotoData(pictureData); } } System.out.println(cauHoi.toString());
 * cauHoi.setScript(fmt.formatCellValue(currentRow.getCell(9)));
 * listCauHoi.add(cauHoi); } workbook.close(); return listCauHoi; } catch
